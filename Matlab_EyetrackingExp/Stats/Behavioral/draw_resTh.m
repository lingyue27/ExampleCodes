function draw_resTh(sub)
% ----------------------------------------------------------------------
% draw_resTh(sub)
% ----------------------------------------------------------------------
% Goal of the function :
% Draw resutls of threshold experiment
% ----------------------------------------------------------------------
% Input(s):
...
% Project : PositionGhost
% Version : 1.0
% ----------------------------------------------------------------------

fprintf(1,'\n\t>> Drawing fit data for threshold [IN PROGRESS]...\n');
...
fileRes  = configEyeAll{end}.tabRes;
const    = configAll{1}.const;

f=figure;
name = sprintf('%s threshold',sub.ini);
...
fileName = sprintf('%s_SacTH',sub.ini);
...
axisYTitle = 'Performance (%)';         % y axis title
...
opt.searchGrid.gamma  = 0.5;                                    % Guess-rate
guess_rate = 0;
...
for tRow = 1:figSet.numRow
    ...
    for tCol = 1:figSet.numCol
        ...
        if (tRow == 1 || tRow == 2)
            ...
            switch tRow
                case 1; rand7Val = 1; titleRow = 'Gap = 0 ms';      fileRes  = configEyeAll{end}.tabRes; colPlot = red;
                case 2; rand7Val = 2; titleRow = 'Gap = 200 ms';    fileRes  = configEyeAll{end}.tabRes; colPlot = blue;
            end
            switch tCol
                case 1; rand5Val = 1;  rand6Val = 1;    titleCol = sprintf('ref = iso / probe = iso / %s',titleRow);           desiredSample   = 30;% desired amount of trial per dot
                case 2; rand5Val = 1;  rand6Val = 2;    titleCol = sprintf('ref = iso / probe = non-iso / %s',titleRow);       desiredSample   = 30;% desired amount of trial per dot
                case 3; rand5Val = 2;  rand6Val = 1;    titleCol = sprintf('ref = non-iso / probe = iso / %s',titleRow);       desiredSample   = 30;% desired amount of trial per dot
                case 4; rand5Val = 2;  rand6Val = 2;    titleCol = sprintf('ref = non-iso / probe = non-iso / %s',titleRow);   desiredSample   = 30;% desired amount of trial per dot
            end
        ...
        elseif tRow == 3
            ...
            switch tCol
                case 1; fileRes  = configEyeAll{end}.tabResGap;
                    gapVal  = 1;
                    matRes  = fileRes(fileRes(:,1) == gapVal,:);
                    titleCol = 'gap 0 ms';                      desiredSample   = 120;
                    colPlot = red;
                case 2; fileRes  = configEyeAll{end}.tabResGap;
                    gapVal  = 2;
                    matRes  = fileRes(fileRes(:,1) == gapVal,:);
                    titleCol = 'gap 200 ms';                    desiredSample   = 120;
                    colPlot = blue;
                case 3; fileRes  = configEyeAll{end}.tabResCong;
                    congVal = 1;
                    matRes  = fileRes(fileRes(:,1) == congVal,:);
                    titleCol = 'Ref-Probe congruent';           desiredSample   = 120;
                    colPlot = green;
                case 4; fileRes  = configEyeAll{end}.tabResCong;
                    congVal = 2;
                    matRes  = fileRes(fileRes(:,1) == congVal,:);
                    titleCol = 'Ref-Probe incongruent';         desiredSample   = 120;
                    colPlot = green;
            end
        elseif tRow == 4
            resCol      = 3;    % performance
            ...
            colPlot     = purple;
            switch tCol
                case 1; fileRes  = configEyeAll{end}.tabResLumRef;
                    lumRefVal = 1;
                    matRes  = fileRes(fileRes(:,1) == lumRefVal,:); % ref iso
                    titleCol = 'Ref lumimance = iso';           desiredSample   = 120;
                case 2; fileRes  = configEyeAll{end}.tabResLumRef;
                    lumRefVal = 2;
                    matRes  = fileRes(fileRes(:,1) == lumRefVal,:);
                    titleCol = 'Ref lumimance = non-iso';       desiredSample   = 120;
                case 3; fileRes  = configEyeAll{end}.tabResLumProbe;
                    lumProbeVal = 1;
                    matRes  = fileRes(fileRes(:,1) == lumProbeVal,:);
                    titleCol = 'Probe lumimance = iso';         desiredSample   = 120;
                case 4; fileRes  = configEyeAll{end}.tabResLumProbe;
                    lumProbeVal = 2;
                    matRes  = fileRes(fileRes(:,1) == lumProbeVal,:);
                    titleCol = 'Probe lumimance = non-iso';     desiredSample   = 120;
            end
        ...
        elseif tRow == 5
            ...
            fileRes  = configEyeAll{end}.tabResGapRef;
            switch tCol
                case 1;
                    lumRefVal = 1;
                    gapval    = 1;
                    titleCol  = 'Ref lumimance = iso / gap 0';       desiredSample   = 60;
                    colPlot   = red;
                case 2;
                    lumRefVal = 1;
                    gapval    = 2;
                    titleCol  = 'Ref lumimance = iso / gap 200';     desiredSample   = 60;
                    colPlot   = blue;
                case 3;
                    lumRefVal = 2;
                    gapval    = 1;
                    titleCol = 'Ref lumimance = non-iso / gap 0';   desiredSample   = 60;
                    colPlot   = red;
                case 4;
                    lumRefVal = 2;
                    gapval    = 2;
                    titleCol = 'Ref lumimance = non-iso / gap 200'; desiredSample   = 60;
                    colPlot   = blue;
            end
        ...
        elseif tRow == 6
            ...
            angCol      = 5;    % played test angle
            numCol      = 6;    % number of trials
            fileRes  = configEyeAll{end}.tabResGapProbe;
            switch tCol
                case 1; 
                    lumProbeVal = 1;
                    gapval    = 1;
                    titleCol  = 'Probe lumimance = iso / gap 0';       desiredSample   = 60;
                    colPlot   = red;
                case 2;
                    lumProbeVal = 1;
                    gapval    = 2;
                    titleCol  = 'Probe lumimance = iso / gap 200';     desiredSample   = 60;
                    colPlot   = blue;
                case 3;
                    lumProbeVal = 2;
                    gapval    = 1;
                    titleCol = 'Probe lumimance = non-iso / gap 0';   desiredSample   = 60;
                    colPlot   = red;
                case 4;
                    lumProbeVal = 2;
                    gapval    = 2;
                    titleCol = 'Probe lumimance = non-iso / gap 200'; desiredSample   = 60;
                    colPlot   = blue;
            end
        ...
        elseif tRow == 7
            ...
            numCol      = 6;    % number of trials
            colPlot     = purple;
            fileRes  = configEyeAll{end}.tabResCond;
            switch tCol
                case 1; rand5Val = 1;  rand6Val = 1;    titleCol = 'ref = iso / probe = iso';           desiredSample   = 60;% desired amount of trial per dot
                case 2; rand5Val = 1;  rand6Val = 2;    titleCol = 'ref = iso / probe = non-iso';       desiredSample   = 60;% desired amount of trial per dot
                case 3; rand5Val = 2;  rand6Val = 1;    titleCol = 'ref = non-iso / probe = iso';       desiredSample   = 60;% desired amount of trial per dot
                case 4; rand5Val = 2;  rand6Val = 2;    titleCol = 'ref = non-iso / probe = non-iso';   desiredSample   = 60;% desired amount of trial per dot
            end
        ...
        text(rangeX/2,ytextAxisX-rangeY*ratioTitleText/2,axisXTitle,'HorizontalAlignment','center');
        ...
        % threshold text + save
        textTH      = sprintf('TH(%1.2f) = %2.1f deg',opt.cutparam(1),resFit.cuts(1));
        textSlope   = sprintf('JND = %2.1f',jnd_val);
        ...
        text(minX-rangeX*ratioScale,maxY+rangeY*ratioTitleText*0.7,textSlope,'HorizontalAlignment','left');
        text(minX-rangeX*ratioScale,maxY+rangeY*ratioTitleText*0.4,textGOF,'HorizontalAlignment','left');

        if isinf(resFit.cuts(1))
            resFit.cuts(1) = NaN;
        end
        fprintf(1,'\n\t   %s: TH(%1.2f) = %2.1f deg; JND = %1.2f',titleCol,opt.cutparam(1),resFit.cuts(1),jnd_val);
        
        if (tRow == 1 || tRow == 2)
            thSave = [thSave;rand5Val,rand6Val,rand7Val,resFit.cuts(1),resFit.SEcuts,jnd_val,resFit.SEslop,resFit.GOF.Dev,resFit.GOF.pDev];
        elseif tRow == 3
            if tCol == 1 || tCol == 2
                thSaveGap = [thSaveGap;gapVal,resFit.cuts(1),resFit.SEcuts,jnd_val,resFit.SEslop,resFit.GOF.Dev,resFit.GOF.pDev];
            elseif tCol == 3 || tCol == 4
                thSaveCong = [thSaveCong;congVal,resFit.cuts(1),resFit.SEcuts,jnd_val,resFit.SEslop,resFit.GOF.Dev,resFit.GOF.pDev];
            end
        elseif tRow == 4
            if tCol == 1 || tCol == 2
                thSaveLumRef = [thSaveLumRef;lumRefVal,resFit.cuts(1),resFit.SEcuts,jnd_val,resFit.SEslop,resFit.GOF.Dev,resFit.GOF.pDev];
            elseif tCol == 3 || tCol == 4
                thSaveLumProbe = [thSaveLumProbe;lumProbeVal,resFit.cuts(1),resFit.SEcuts,jnd_val,resFit.SEslop,resFit.GOF.Dev,resFit.GOF.pDev];
            end
        elseif tRow == 5
            thSaveGapRef = [thSaveGapRef; lumRefVal, gapval, resFit.cuts(1),resFit.SEcuts,jnd_val,resFit.SEslop,resFit.GOF.Dev,resFit.GOF.pDev];
        elseif tRow == 6
            thSaveGapProbe = [thSaveGapProbe; lumProbeVal, gapval, resFit.cuts(1),resFit.SEcuts,jnd_val,resFit.SEslop,resFit.GOF.Dev,resFit.GOF.pDev];
        elseif tRow == 7
            thSaveCond = [thSaveCond; rand5Val, rand6Val, resFit.cuts(1),resFit.SEcuts,jnd_val,resFit.SEslop,resFit.GOF.Dev,resFit.GOF.pDev];
        end
        ...
        text(minX+rangeX*0.78,minY+rangeY*0.1,sprintf('%i trials',desiredSample),'HorizontalAlignment','left','VerticalAlignment','middle');
        text(minX+rangeX*0.78,minY+rangeY*0,sprintf('%i trials',desiredSample/2),'HorizontalAlignment','left','VerticalAlignment','middle');
...
configEyeAll{end}.thSave            = thSave;
configEyeAll{end}.thSaveGap         = thSaveGap;
configEyeAll{end}.thSaveCong        = thSaveCong;
configEyeAll{end}.thSaveLumRef      = thSaveLumRef;
configEyeAll{end}.thSaveLumProbe    = thSaveLumProbe;
configEyeAll{end}.thSaveGapRef      = thSaveGapRef;
configEyeAll{end}.thSaveGapProbe    = thSaveGapProbe;
configEyeAll{end}.thSaveCond        = thSaveCond;
